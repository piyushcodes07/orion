...
class SendEmailInput(BaseModel):
    ...
    """
    email_id: str = Field(
        description="Gmail message ID to reply to. This must be a valid Gmail message ID obtained from the fetch_emails_tool or could be a receipient email address that user wants to send a new email. if your replying to a email then use the emails message id, else you wanna use a valid receipient email address"
    )
    ...
    additional_recipients: Optional[List[str]] = Field(
        default=None,
        description="Optional additional recipients to include. if you have more than one email address then the addresses other than the first one should be passed in additional_receipients"
    )
