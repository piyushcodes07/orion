    ...
    if last_message.tool_calls:
        for tool_call in last_message.tool_calls:
            if tool_call["name"] == "Done":
                # Create tool message for the Done tool call
                tool_message = {
                    "role": "tool",
                ...
                # Update state with tool message and send to mark_as_read_node
                #
                new_state = {
                    "email_input":state['email_input'],
                    "messages": state['messages'][-1]+tool_msg
                }

                return [Send("mark_as_read_node", new_state)]
            else:
