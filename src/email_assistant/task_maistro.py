...
def update_profile(state: State, config: RunnableConfig, store: BaseStore):
    ...
    updated_messages=list(merge_message_runs(messages=[SystemMessage(content=TRUSTCALL_INSTRUCTION_FORMATTED)] + state["task_messages"][:-1]))
    ...
    tool_calls = state['task_messages'][-1].tool_calls
    # Return tool message with update verification
    return {"task_messages": [{"role": "tool", "content": "updated profile", "tool_call_id":tool_calls[0]['id']}]}
...
def update_todos(state: State, config: RunnableConfig, store: BaseStore):
    ...
    return {"task_messages": [{"role": "tool", "content": todo_update_msg, "tool_call_id":tool_calls[0]['id']}]}
...
def update_instructions(state: State, config: RunnableConfig, store: BaseStore):
    ...
    new_memory = model.invoke([SystemMessage(content=system_msg)]+state['task_messages'][:-1] + [HumanMessage(content="Please update the instructions based on the conversation")])
